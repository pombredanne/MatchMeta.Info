...
class MalwarePspCidMeta(obj.ProfileModification):
...
class PsXviewMeta(common.AbstractWindowsCommand, sessions.SessionsMixin):
    ...
    def render_text(self, outfd, data):
        ...
        cursor.execute("SELECT FileName FROM MetaInfo")
        ...
        for offset, process, ps_sources in data:
            ...
            arraySimpleRatio = []
            FileName = process.ImageFileName.lower().strip()
            cursor.execute("SELECT FileName FROM MetaInfo WHERE FileName LIKE:FileName", {"FileName":FileName})
            ...
            if exists is not None:
                ...
                exact = ""
                for row in rows:
                	SimpleRatio = fuzz.ratio(row[0],FileName)
               		if SimpleRatio > 94:         
				arraySimpleRatio.append(row[0])

            like = len(arraySimpleRatio)
